...
```sql
SELECT nome, LOWER(SPLIT_PART(nome, ' ', 1)) AS login
FROM vendedor
```
...
```sql
SELECT descricao, ROUND((qtde * preco), 2) AS valor_total_estoque
FROM produto;
```
...
```sql
SELECT nome, AGE(CURRENT_DATE, data_nasc) AS aniversario
FROM cliente;
```
...
```sql
SELECT nome, SUBSTRING(cpf FROM 1 FOR 3) || '********'  AS cpf_oculto
FROM cliente;
```
...
```sql
SELECT nome, salario, comissao, TRUNC((salario + comissao), 0) AS salario_total
FROM vendedor;
```
...
```sql
SELECT UPPER(descricao), LENGTH(descricao) AS tamanho
FROM produto;
```
...
```sql
SELECT id_produto, ABS(qtde - qtde_min) AS diferenca
FROM produto; 
```
...
```sql
SELECT CONCAT('O vendedor(a) ', nome , ' nasceu no ano de ' ,
